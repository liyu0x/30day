...
void cmd_exit(struct CONSOLE *cons, int *fat)
{
    ...
    if (cons->sht != 0)
    {
        fifo32_put(fifo, cons->sht - shtctl->sheets0 + 768); // 768~1023 通知其他任务执行close_console
    }
    else
    {
        fifo32_put(fifo, cons->sht - shtctl->sheets0 + 1024); // 1024~2023
    }
    io_sti();
    for (;;)
    {
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
    ...
    if (finfo != 0)
    {
        ...
        if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00)
        {
            ...
            task->ds_base = (int)q;
            set_segmdesc(gdt + task->sel / 8 + 1000, finfo->size - 1, (int)p, AR_CODE32_ER + 0x60); // 代码段
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
    ...
    else if (edx == 5)
    {
        ...
        sheet_updown(sht, shtctl->top); /* 将窗口图层高度指定为鼠标当前所在图层高度, 鼠标移到上层 */
        reg[7] = (int)sht;
    }
    else if (edx == 6)
    {
    ...
    else if (edx == 20)
    {
         ...
         * 蜂鸣器发声
         * EDX=20
         * EAX=声音频率（单位是mHz，即毫赫兹）例如当EAX=4400000时，则发出440Hz的声音频率设为0则表示停止发声
         **/
        if (eax == 0)
        {
            // 关闭蜂鸣器
