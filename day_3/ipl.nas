...
retry:
		...
		MOV BX,0			
		MOV	DL,0x00			; A驱动器
		INT 0x13			; 调用磁盘BIOS, 会返回一个值，如果没有错就返回0，否则返回1
		JNC next			; 没出错的话跳转到next
		ADD SI,1			; 往SI加1
		CMP SI,5			; 比较SI与5
		JAE error			; SI >= 5时, 跳转到ERROR

		MOV AH,0x00			; AH=0x00: 重置软盘状态
		MOV DL,0X00			; A驱动器
		INT 0x13			; 重置驱动器
		JMP retry
...
next:
		MOV AX,ES			
		ADD AX,0x0020		; 把内存地址后移0x200(即512个字节，一个扇区)
		MOV ES,AX			; 段寄存器无法直接作为操作数，所以这里使用了AX
		ADD CL,1			; 往CL里加1
		CMP CL,18			; 比较CL与18
...
; JNC(jump if not carry): 如果进位标志(carry flag)是1, 就跳转

; JAE(jump if above or equal): 大于等于时跳转

; JBE(jump if below or equal): 小于等于时跳转
