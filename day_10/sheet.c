...
void sheet_slide(struct SHTCTL *ctl, struct SHEET *sht, int vx0, int vy0)
{
    ...
    if (sht->height >= 0)
    {                                                                                          /* 如果正在显示, 则按图层刷新画面 */
        sheet_refreshsub(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize); // 刷新移动前的位置
        sheet_refreshsub(ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize);                 // 刷新移动后的位置
    }
    return;
}
...
void sheet_free(struct SHTCTL *ctl, struct SHEET *sht)
{
    if (sht->height >= 0)
    {
        sheet_updown(ctl, sht, -1); /* 如果正在显示,则设为隐藏 */
    }
    sht->flags = 0; /* "未使用" 状态 */
    return;
}
